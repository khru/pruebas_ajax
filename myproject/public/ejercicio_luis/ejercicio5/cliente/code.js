...
function checkNick() {
	if(request.readyState == READY_STATE_COMPLETE) {	    
	    if(request.status == 200) {
	    	...
	    	if(json_response.respuesta.disponible == 'yes'){
	    		text = "<span style='color:green'>✓<span>";
	    	}else{
	    		text = "<p>El usuario no está disponible</p><p>Alternativas:</><ul>";
	    		var alternativas = json_response.respuesta.alternativas;
	    		for(var i = 0; i<alternativas.length; i++){
	    			text += "<li style='cursor:pointer;margin-bottom:5px' id='" + i + "'>" + alternativas[i] + "</li> ";
	    		}
	    		text += "</ul>";
	    	}
	    	//Display response
...
function sendOnBlur(){
	sendRequest('GET', '../../ejercicio5/servidor/checkNick.php?nick='+login.value, checkNick);
}
